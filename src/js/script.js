...
(function buildPictureGrid() {
  const IMAGES_DIR = './src/media/pictures';
  const TOTAL = 33;
  const PER_ROW = 3;
  const container = document.getElementById('pictures_list');
  if (!container) return;

  // Clear any hardcoded rows/images
  container.innerHTML = '';

  const rows = Math.ceil(TOTAL / PER_ROW); // should be 11

  for (let r = 0; r < rows; r++) {
    const rowEl = document.createElement('div');
    rowEl.className = 'row';

    for (let c = 1; c <= PER_ROW; c++) {
      const idx = r * PER_ROW + c;
      if (idx > TOTAL) break;

      const img = document.createElement('img');
      const src = `${IMAGES_DIR}/${idx}.jpeg`;

      img.src = src;
      img.alt = `Portfolio image ${idx}`;
      img.loading = 'lazy';
      img.decoding = 'async';
      img.referrerPolicy = 'no-referrer'; // optional, safer for some hosts
...
function calculateAge() {
    const birthDate = new Date(2002, 1, 24); // February 24, 2002 (month is 0-indexed btw so 1 is correct)
    const now = new Date();
